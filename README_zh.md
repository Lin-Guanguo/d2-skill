...
这个项目为 AI Agent 提供一个安全、可审计的 Destiny 2 数据桥接层。它可以回答装备从哪里来、检查你拥有的物品和 roll、查看账号进度、查询活动历史，并执行狭窄的物品操作，例如转移、装备、锁定和免费插槽插入。CLI 返回结构化 JSON 事实和安全操作计划；主观判断留给 Agent 和用户。

## 能做什么

- 查询官方物品信息、来源分类、当前商人路线、商人成本、是否买得起，以及 engram 预览池路线。
- 检查角色和仓库里的已有物品，包括 perk、属性、插槽、重复分组和 DIM wishlist 证据。
...
OAuth token、wishlist cache、profile cache 和命令审计日志都会存放在仓库外的 `~/.d2-skill/`。
...
- `d2-items`: 已拥有物品、roll 和 wishlist 证据、重复物品 review、转移、安全装备操作、插槽，以及只读游戏内配装。
...
- 清理工作流使用重复分组、wishlist 证据、物品检查和安全转移原语。用户在游戏内处理拆解。
...
- `src/inventory/`, `src/items/`, `src/wishlist/`: 已拥有物品事实和 roll 证据。
- `src/gear/`, `src/sockets/`, `src/loadouts/`: 安全物品操作、插槽检查和只读游戏内配装。
